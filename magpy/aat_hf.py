...
class AAT_HF(object):
    ...
    def compute(self, R_disp, B_disp, e_conv=1e-13, r_conv=1e-13, maxiter=400, max_diis=8, start_diis=1, print_level=0):
        ...
        for B in range(3):
            ...
            escf_B_pos, C_B_pos = scf0.solve_scf(e_conv, r_conv, maxiter, max_diis, start_diis, print=print_level)
            ...
            strength[B] = -B_disp
            H.add_field(field='magnetic-dipole', strength=strength)
            escf_B_neg, C_B_neg = scf0.solve_scf(e_conv, r_conv, maxiter, max_diis, start_diis, print=print_level)
            ...
            for R in range(3*self.molecule.natom()):
                ...
                pp = det_overlap(C_R_pos[:,o], H_pos.basisset, C_B_pos[:,o], H.basisset).imag
                pm = det_overlap(C_R_pos[:,o], H_pos.basisset, C_B_neg[:,o], H.basisset).imag
                mp = det_overlap(C_R_neg[:,o], H_neg.basisset, C_B_pos[:,o], H.basisset).imag
                mm = det_overlap(C_R_neg[:,o], H_neg.basisset, C_B_neg[:,o], H.basisset).imag
